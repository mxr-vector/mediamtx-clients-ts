// @ts-nocheck
// Derived from MediaMTX v1.19.0 internal/servers/moq/reader.js.
"use strict";
 ...
 * @property {boolean} enableAudio - whether audio tracks should be subscribed.
 * @property {number} maxVideoFps - maximum video decode frame rate. 0 disables throttling.
 * @property {number} maxRenderWidth - maximum internal canvas width. 0 keeps source width.
 * @property {number} maxRenderHeight - maximum internal canvas height. 0 keeps source height.
 * @property {number} maxVideoDecodeQueueSize - maximum queued VideoDecoder frames before dropping.
 * @property {boolean} debug - whether debug logs should be printed.
...
export class MediaMTXMoQReader {
  static #RETRY_PAUSE = 5000;
  static #MAX_RETRY_PAUSE = 30000;
  ...
  #conf;
  #state = "running";
  #retryPause = MediaMTXMoQReader.#RETRY_PAUSE;
  #restartTimeout = null;
  #wt = null;
  #fingerprint = null;
  #catalog = null;
  ...
  #handleError(err) {
    if (this.#state === "running") {
      ...
      const retryPause = this.#retryPause;
      this.#restartTimeout = window.setTimeout(() => this.#restart(), retryPause);
      this.#retryPause = Math.min(
        this.#retryPause * 2,
        MediaMTXMoQReader.#MAX_RETRY_PAUSE,
      );
      ...
      if (this.#conf.onError !== undefined) {
        this.#conf.onError(`${err}, retrying in ${Math.round(retryPause / 1000)} seconds`);
      }
    }
  }
  ...
  async #connect() {
    ...
    await this.#wt.ready;
    this.#retryPause = MediaMTXMoQReader.#RETRY_PAUSE;
    this.#log("connected");
  ...
  async #subscribeAllTracks() {
    ...
    for (let i = 0; i < this.#catalog.tracks.length; i++) {
      ...
      if (/^(avc3|hev1|av01|vp09|vp8)/.test(track.codec)) {
        ...
        if (this.#conf.enableAudio && this.#audioTrack === null) {
  ...
  async #subscribeTrack(requestId, track) {
    ...
    if (requestId === MediaMTXMoQReader.#VIDEO_REQUEST_ID) {
      ...
      let ctx2D = null;
      if (ctxGL === null) {
        this.#log("video renderer: 2D canvas");

        ctx2D = this.#videoCanvas.getContext("2d");
      } else {
        this.#log("video renderer: WebGL2");
    ...
    this.#log("subscribed track " + track.name + " (" + track.codec + ")");
  }
  ...
  #decodeVideo(data, groupId) {
    ...
    if (/^(avc3)/.test(this.#videoTrack.codec)) {
      ...
      chunkType = hasIdr ? "key" : "delta";

      if (sps !== null && pps !== null) {
        const paramsChanged = this.#videoParams === null ||
        ...
        if (paramsChanged) {
          ...
          this.#videoConfigured = true;
          this.#videoNeedsKeyFrame = true;
          this.#log("video params updated");
        }
      }

      if (!this.#videoConfigured || (this.#videoNeedsKeyFrame && !hasIdr)) {
        this.#log("skipping AVC delta frame before key frame/config");
        return;
      }
      ...
      if (!this.#videoConfigured || (this.#videoNeedsKeyFrame && !hasIrap)) {
        this.#log("skipping HEVC delta frame before key frame/config");
        return;
      }
    }
    ...
    // this happens when the screen is off or decoding cannot keep up.
    if (this.#videoDecoder.decodeQueueSize >= this.#conf.maxVideoDecodeQueueSize) {
      this.#log("skipping video frame, decode queue is full");
      return;
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      if (message.includes("key frame is required")) {
        this.#videoNeedsKeyFrame = true;
        this.#log("waiting for video key frame after decoder configure");
        return;
      }
      this.#handleError(message);
      return;
  ...
  #renderSize(sourceWidth, sourceHeight) {
    const maxWidth = this.#conf.maxRenderWidth || sourceWidth;
    const maxHeight = this.#conf.maxRenderHeight || sourceHeight;
  ...
  #decodeAudio(data, groupId) {
    ...
    if (
      ...
      this.#log("skipping audio frame, decode queue is full");
      return;
    }
  ...
  static #Reorderer = class {
    ...
    constructor(maxReorderered, log) {
    ...
    push(data, groupId) {
      if (this.#curGroupId === null) {
        ...
        this.#log("skipping out-of-order subgroup");
      } else if (
        ...
        if (countInRange === diff) {
          ...
          this.#log("too many reordered subgroups, flushing");
          return this.#flushUpTo(groupId);
        }
      }
      return [];
